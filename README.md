...
* In Details view, the default **Name** column now starts at roughly two-thirds of the list container width on fresh/default column layouts
...
* Protected-folder paste now uses Explorer++'s own `IFileOperation` copy/move path for normal filesystem clipboard items, so operations that require elevation can go straight to the UAC prompt without the extra Windows "Continue" confirmation first
* Protected-folder delete now uses the same direct-elevation file-operation behavior, so deleting files/folders there should go straight to UAC without the extra Windows "Continue" confirmation first
